<script lang="ts">
  // Read-only quick view for a remote text file. Opened from SftpPane on
  // Enter / the eye button; the backend already had SftpReadPreview (a
  // capped base64 read), this is the UI that was missing.
  //
  ...
  // the button would visibly do nothing. The modal is created per file
  // (keyed by the {#if} in SftpPane), so mounting is the right trigger.
  //
  // Focus must move into the modal: the overlay blocks clicks, but keyboard
  // focus would otherwise stay in a background terminal's textarea and every
  // keystroke would land in that SSH session.
  onMount(() => {
    load();
    // Focus the scrolling body, not the dialog shell: clicking text inside
    // a tabindex=-1 ancestor refocuses that ancestor and the browser scrolls
    // it into view, snapping a long file back to the top. The body is the
  ...
  // Single exit point: the X, the overlay and Escape all land here so an
  // accidental click cannot throw away unsaved work.
  ...
  function onKey(e: KeyboardEvent) {
    // Ctrl+S saves from anywhere in the modal, including inside the textarea.
    ...
    if (e.key === "Escape") {
      e.preventDefault();
      // While editing, Escape backs out of the edit (with a confirm if
      // there are changes) rather than throwing the whole modal away.
    ...
    <header>
      ...
      <div class="header-actions">
        {#if !isBinary && !loading && !error}
          ...
          {/if}
        {/if}
        <button class="close" onclick={requestClose} title="Close (Esc)">✕</button>
      </div>
    </header>
